...
if __name__ == "__main__":
    ...
    for seed in range(0, 15):  # Arbitrary 15 msgs sent
        ...
        wireObj.send_request(Command.PUT, str(seed), len(value_to_send), value_to_send, 0, .2)  # send to node 0
    ...
    for seed in range(14, -1, -1):  # Arbitrary 14 msgs sent
        ...
        wireObj.send_request(Command.GET, str(seed), len(value_to_send), value_to_send, 0, .2)  # send to node 0
        response_code, value = wireObj.receive_reply()
        print print_response(response_code)
