...
if __name__ == "__main__":
    ...
    wireObj.send_request(Command.SHUTDOWN, str('app'), len(value_to_send), value_to_send, threading.currentThread(),
    ...
    wireObj.send_request(Command.PUT, str('app'), len(value_to_send), value_to_send, threading.currentThread(),
                         1, timeout, retrials=retrial_times)  # send to node 0
    response_code, value = wireObj.receive_reply(threading.currentThread(), Command.PUT)
    print print_response(response_code) + "\n"

    wireObj.send_request(Command.GET, str('app'), len(value_to_send), value_to_send, threading.currentThread(),
                         1, timeout, retrials=retrial_times)  # send to node 0
    response_code, value = wireObj.receive_reply(threading.currentThread(), Command.PUT)
    print print_response(response_code) + "\n"
