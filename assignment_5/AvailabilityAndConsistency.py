...
def search(key, retrieve_mode="ping"): # TO-Do create class settings.aliveNessTable
    ...
    while True:
        ...
        if settings.aliveNessTable.isAlive(str(cursor)):
            return cursor
...
def successor(node_id, retrieve_mode="ping"):
    cursor = target_node = int(node_id)
    while True:
        # decrement
        cursor = prev_node(cursor)
        ...
        # Full loop
        if cursor == int(int(settings.hashedKeyModN)):
            return cursor

        # For distance > 3 Check if the cursor in the aliveness table first. If not alive, to Try to PING the cursor
        if (target_node > cursor and target_node - cursor > 3) or \
                (target_node < cursor and (int(settings.N) - cursor) + target_node > 3) or retrieve_mode == 'table':
                time.sleep(.2)
                if settings.aliveNessTable.isAlive(str(cursor)):
                    return cursor

        else:
            wire_obj = wire.Wire(int(settings.N), target_node, settings.mode, "main")
            # Print.print_("Searching for Node[PING]:  " + str(cursor), Print.AvailabilityAndConsistency, target_node, threading.currentThread())
...
def predecessor(node_id, retrieve_mode="ping"):
    ...
    while True:
        ...
        # For distance > 3 Check if the cursor in the aliveness table first. If not alive, to Try to PING the cursor
        if (cursor > target_node and cursor - target_node > 3) or \
                (cursor < target_node and (int(settings.N) - target_node) + cursor > 3) or retrieve_mode == 'table':
                # Print.print_("Searching for Node[table]:  " + str(cursor) +
                #             ", Status: " + str(settings.aliveNessTable.get(str(cursor))),
                #              Print.AvailabilityAndConsistency, target_node, threading.currentThread())
...
def get_healthy_successor(key, node_id=""):
    s = [-1, -1]
    if node_id == "":
        node_id = hash(key) % int(settings.N)
    s[0] = successor(node_id, 'table')
    # print "successor of", node_id, "is", s[0]
    # if s[0] != -1 and s[0] != int(settings.hashedKeyModN):
    s[1] = successor(s[0], 'table')
    # print "successor of", s[0], "is", s[1]
